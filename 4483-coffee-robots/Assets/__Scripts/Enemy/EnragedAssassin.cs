...
public class EnragedAssassin : MonoBehaviour, IEnemy
{
    [Header("GameObjects")]
    ...
    void Move(Transform target, float distance)
    {
        ...
        {
            transform.position = Vector3.MoveTowards(transform.position, new Vector3(target.transform.position.x, transform.position.y, target.transform.position.z), Config.enragedAssassinMovementSpeed * Config.difficultyMovementMod * Time.deltaTime * -1);
        }
        else if (distance > Config.assassinMaxRange) // move towards player
        {
            transform.position = Vector3.MoveTowards(transform.position, new Vector3(target.transform.position.x, transform.position.y, target.transform.position.z), Config.enragedAssassinMovementSpeed * Config.difficultyMovementMod * Time.deltaTime);
        }
    }

    void Attack()
    {
        if (Time.time > lastAttackTime + (1/Config.enragedAssassinAttackRate))
        {
            ...
            dart.Shoot(firePoint.transform.forward, Config.assassinProjectileSpeed, true);
        }
    }
}
